...
- **Note for whoever swaps in the custom domain:** `og:url`, `og:image`, `twitter:image`, the canonical link, the JSON-LD `url`, `sitemap.xml`, and `robots.txt`'s `Sitemap:` line all currently hardcode `https://glnewsmapping.github.io/Global-News-Daily/` — all of them need updating together, not just `og:url`, or the canonical/JSON-LD/sitemap will keep pointing at the old GitHub Pages path after the domain goes live.
