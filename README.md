...
- **Conflict** -- real violence only: root codes `18, 19, 20` in full (assault, armed clash, mass violence) plus the specific violent sub-codes of two otherwise non-violent roots -- `145`/`1451-1454` ("protest violently, riot", as opposed to peaceful marches/strikes/boycotts under the same PROTEST root) and `175` ("use tactics of violent repression", as opposed to arrests/curfews/property seizure/deportation under the same COERCE root). Real individual incidents, not a yearly country total. The 10 stories with the most corroborating articles are always included in the final selection regardless of region spread (see `guaranteed_top` in `balance_by_region`) -- a real spike in coverage shouldn't lose out to spreading picks across regions.
