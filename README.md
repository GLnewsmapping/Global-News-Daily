...
The real meta description, when present, is kept as a `lede` field separate from the GDELT-derived `summary` metadata list (type/actors/location/etc.) and rendered above it in its own, more prominent style in `index.html` -- it's the one piece of real prose in an otherwise terse metadata block, so it's styled to read that way rather than as just another bullet. `_lede_from_description()` drops it entirely when it's just a restatement of the headline (common with CMS templates that default the meta description to the title verbatim) and truncates on a word boundary rather than mid-word.

**Both are additionally relevance-filtered** on the real article content: high-news-volume conflict-zone/geopolitically-active countries (China, India, Pakistan, Nigeria, Colombia...) generate far more everyday news than actual conflict or diplomatic coverage, and GDELT's event codes can't tell the difference -- confirmed live, this was letting through airline PR, stock-trade reports, a decades-old criminal sentencing, a zombie-movie culture piece, and even a Country Music Association Awards story, all tagged as conflict or political purely because of where they were geolocated. `CONFLICT_RELEVANCE_KEYWORDS` / `POLITICAL_RELEVANCE_KEYWORDS` (word-boundary matched, not substring -- a naive substring check would match "fighters" inside "Firefighters") require the real title/description to actually mention war/violence or diplomacy/geopolitics respectively before a story survives. Because most candidates don't survive this (confirmed: usually 15-25% for conflict, ~20-30% for political), both fetch a *larger* raw pool (`--target-locations` × 2) and filter it *before* capping down to the display target, rather than after -- filtering an already-small capped set would leave far too few stories. A fetch that fails or times out leaves the story's original synthesized label in place rather than dropping it -- benefit of the doubt when a site simply couldn't be reached, not a penalty.
...
## Daily brief pages (SEO)

`index.html` is entirely WebGL/client-rendered -- a search crawler gets no indexable text from it, and a social-media link unfurler can't screenshot a `<canvas>`. `generate_daily_brief()` closes that gap using the exact same per-day data as the archive above: a plain static `brief/YYYY-MM-DD.html` per day (real headlines, real `lede` text, the same metadata list shown on the live site, grouped by category), a `brief/index.html` linking all of them, and a regenerated `sitemap.xml` covering the homepage plus every current brief page. Pruned on the same `--archive-days` window as the archive snapshots, so a brief page and its sitemap entry never outlive the data behind them. `--brief-dir` changes the folder, `--no-brief` skips this entirely. The scheduled GitHub Action commits `brief/` and `sitemap.xml` alongside everything else.
...
The same file also carries the three Arctic routes from the Arctic Routes tab (`volume: "arctic"`), rendered in red rather than the CIA set's gold. Their waypoints are traced through each route's real named straits (Kara Strait, Vilkitsky Strait, Long Strait for the NSR; Lancaster Sound, Barrow Strait, Viscount Melville Sound, M'Clure Strait for the NWP; Fram Strait for the TSR), coordinates checked against Wikipedia. Checking only the waypoints against `data/country_polygons.json` wasn't enough -- a straight line between two verified-water points can still cut across an island in between, and several did (Severnaya Zemlya, Banks Island, the Alaska coast). Every *segment* is now sampled at 50 points and re-verified as water end to end; the two spots too narrow for Natural Earth's 1:110m coastline data to resolve as open water at all (the Vilkitsky Strait pinch and the New Siberian Islands stretch) were routed around using a real water-only pathfind over the local coastline data, not hand-picked.
...
- `index.html` has a favicon, meta description, canonical link, a `WebSite` JSON-LD block, and Open Graph/Twitter card tags — including `img/og-card.png` (a static 2400×1260 card generated to match the site's own type/color system, since the globe itself is WebGL and can't be screenshotted by a social-media link unfurler) so shared links get a proper title/description/preview image instead of a bare link. A `<noscript>` fallback explains the site needs JavaScript rather than showing a blank page. Plus an "About & data sources" link in the sidebar that opens a modal summarizing each data source. `robots.txt`, `sitemap.xml`, and a themed `404.html` round out the "real site" basics for GitHub Pages.
- **Note for whoever swaps in the custom domain:** `og:url`, `og:image`, `twitter:image`, the canonical link, and the JSON-LD `url` in `index.html`, plus `robots.txt`'s `Sitemap:` line, all currently hardcode `https://glnewsmapping.github.io/Global-News-Daily/` and need updating by hand. `sitemap.xml` and every brief page's own canonical link do *not* need hand-editing -- they're regenerated each run from the `SITE_BASE_URL` constant at the top of `fetch_news_data.py`, so update that one constant and the next scheduled run fixes all of them together.
- **`globe.gl` is loaded unpinned** (`https://cdn.jsdelivr.net/npm/globe.gl` with no version, unlike `leaflet` which is pinned to `1.9.4`) — it always serves whatever the latest release is, so an upstream breaking change would silently break the live site with no warning and no easy rollback. Pin it to whatever version is current at the time (check via jsdelivr, e.g. `https://cdn.jsdelivr.net/npm/globe.gl@<version>`) before relying on this for real traffic — not changed here since guessing a version number blind risks pinning to one that doesn't exist or is already broken.
...
- The globe's surface texture is `img/earth-blue-marble.jpg`, NASA's Blue Marble Next Generation (5400×2700, self-hosted rather than pulled from three-globe's bundled 4096×2048 example asset) — noticeably sharper ice/coastline detail near the poles, and self-hosting avoids a cross-origin texture load that NASA's own image server doesn't send CORS headers for (confirmed live: an anonymous-mode image load against it fails).
