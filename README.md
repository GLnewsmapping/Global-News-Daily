...
The same file also carries the three Arctic routes from the Arctic Routes tab (`volume: "arctic"`), rendered in red rather than the CIA set's gold. Their waypoints are traced through each route's real named straits (Vilkitsky, Dmitry Laptev, Long Strait for the NSR; Lancaster Sound, Barrow Strait, M'Clure Strait for the NWP; Fram Strait for the TSR), coordinates checked against Wikipedia, and every point verified against `data/country_polygons.json` to confirm it falls in water, not on land -- Natural Earth's 1:110m coastline generalization swallows some of the real (much narrower) straits, so a few waypoints sit slightly off the literal strait midpoint to clear it.
