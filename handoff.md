...
- Added static daily `brief/YYYY-MM-DD.html` pages (`generate_daily_brief()`), built from the same per-day data as `data/archive/`, since the live globe/map is entirely client-rendered WebGL and gives Google nothing to index. `brief/index.html` links all of them, and `sitemap.xml` is now regenerated each run to list all of them plus the homepage, from a single `SITE_BASE_URL` constant -- pruned on the same window as the archive. Seeded now with real pages for all 7 days already in `data/archive/`. Linked from the About modal. `.github/workflows/refresh-data.yml` updated to commit `brief/` and `sitemap.xml`.
- Pre-launch SEO/sharing pass on `index.html`: added a canonical link, a `WebSite` JSON-LD block, a real `og:image`/`twitter:image` (`img/og-card.png`, a static card generated to match the site's own design system since the WebGL globe itself can't be screenshotted by a link-unfurler), upgraded `twitter:card` to `summary_large_image`, and added a `<noscript>` fallback message. **Not done, flagged instead:** `globe.gl`'s CDN script tag is unpinned (always "latest", unlike `leaflet` which is pinned) — needs a real version number from a machine with network access, not a guess. Also flagged: og:url/canonical/JSON-LD/sitemap/robots.txt all still hardcode the GitHub Pages URL and need updating together once the custom domain goes live (see README Notes).
...
6. **Once the custom domain is live:** submit `sitemap.xml` to Google Search Console and request indexing (the new `brief/` pages are the first real crawlable text the site has had -- worth confirming Google actually picks them up rather than assuming). Also pin `globe.gl`'s CDN version and update `SITE_BASE_URL` in `fetch_news_data.py` plus the handful of hardcoded URLs in `index.html`/`robots.txt` listed in the README notes.

Growth, marketing sequencing, and monetization phasing are covered separately in the Growth & Revenue plan artifact from this session (companion to the Launch Plan artifact) -- this list stays focused on code/data next steps.
